"""
Routes for OUTC.csv file validation.
Handles single file, multiple files, folder, and ZIP uploads for OUTC files.
"""
...
from .validation_models import OUTC_SCHEMA

router = APIRouter(prefix="/outc", tags=["OUTC File Validation"])


@router.post("/validate-single")
async def validate_single_outc_file(file: UploadFile = File(...)) -> JSONResponse:
    """
    Validate a single OUTC.csv file.
    ...
    result = validate_single_file(file_bytes, file.filename, OUTC_SCHEMA)
...
async def validate_multiple_outc_files(files: List[UploadFile] = File(...)) -> JSONResponse:
    """
    Validate multiple OUTC.csv files.
    ...
    # Validate all files
    results = validate_multiple_files(file_data, OUTC_SCHEMA)
...
async def validate_outc_zip_file(file: UploadFile = File(...)) -> JSONResponse:
    """
    Validate OUTC.csv files from a ZIP archive.
    ...
    try:
        ...
        # Validate all files
        results = validate_multiple_files(files, OUTC_SCHEMA)
        summary = create_validation_summary(results)
...
async def validate_outc_folder(files: List[UploadFile] = File(...)) -> JSONResponse:
    """
    Validate all OUTC.csv files from a folder upload.
...
@router.get("/schema")
async def get_outc_schema() -> JSONResponse:
    """
    Get the OUTC file schema requirements.
    
    Returns:
        JSONResponse with schema information
    """
    return JSONResponse({
        "file_prefix": OUTC_SCHEMA.file_prefix,
        "required_columns": OUTC_SCHEMA.required_columns,
        "column_types": OUTC_SCHEMA.column_types,
        "example_row": OUTC_SCHEMA.example_row
    })
