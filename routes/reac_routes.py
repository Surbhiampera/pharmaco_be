"""
Routes for REAC.csv file validation.
Handles single file, multiple files, folder, and ZIP uploads for REAC files.
"""
...
from .validation_models import REAC_SCHEMA

router = APIRouter(prefix="/reac", tags=["REAC File Validation"])


@router.post("/validate-single")
async def validate_single_reac_file(file: UploadFile = File(...)) -> JSONResponse:
    """
    Validate a single REAC.csv file.
    ...
    result = validate_single_file(file_bytes, file.filename, REAC_SCHEMA)
...
async def validate_multiple_reac_files(files: List[UploadFile] = File(...)) -> JSONResponse:
    """
    Validate multiple REAC.csv files.
    ...
    # Validate all files
    results = validate_multiple_files(file_data, REAC_SCHEMA)
...
async def validate_reac_zip_file(file: UploadFile = File(...)) -> JSONResponse:
    """
    Validate REAC.csv files from a ZIP archive.
    ...
    try:
        ...
        # Validate all files
        results = validate_multiple_files(files, REAC_SCHEMA)
        summary = create_validation_summary(results)
...
async def validate_reac_folder(files: List[UploadFile] = File(...)) -> JSONResponse:
    """
    Validate all REAC.csv files from a folder upload.
...
@router.get("/schema")
async def get_reac_schema() -> JSONResponse:
    """
    Get the REAC file schema requirements.
    
    Returns:
        JSONResponse with schema information
    """
    return JSONResponse({
        "file_prefix": REAC_SCHEMA.file_prefix,
        "required_columns": REAC_SCHEMA.required_columns,
        "column_types": REAC_SCHEMA.column_types,
        "example_row": REAC_SCHEMA.example_row
    })
