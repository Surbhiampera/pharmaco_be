"""
Routes for DRUG.csv file validation.
Handles single file, multiple files, folder, and ZIP uploads for DRUG files.
"""
...
from .validation_models import DRUG_SCHEMA

router = APIRouter(prefix="/drug", tags=["DRUG File Validation"])


@router.post("/validate-single")
async def validate_single_drug_file(file: UploadFile = File(...)) -> JSONResponse:
    """
    Validate a single DRUG.csv file.
    ...
    result = validate_single_file(file_bytes, file.filename, DRUG_SCHEMA)
...
async def validate_multiple_drug_files(files: List[UploadFile] = File(...)) -> JSONResponse:
    """
    Validate multiple DRUG.csv files.
    ...
    # Validate all files
    results = validate_multiple_files(file_data, DRUG_SCHEMA)
...
async def validate_drug_zip_file(file: UploadFile = File(...)) -> JSONResponse:
    """
    Validate DRUG.csv files from a ZIP archive.
    ...
    try:
        ...
        # Validate all files
        results = validate_multiple_files(files, DRUG_SCHEMA)
        summary = create_validation_summary(results)
...
async def validate_drug_folder(files: List[UploadFile] = File(...)) -> JSONResponse:
    """
    Validate all DRUG.csv files from a folder upload.
...
@router.get("/schema")
async def get_drug_schema() -> JSONResponse:
    """
    Get the DRUG file schema requirements.
    
    Returns:
        JSONResponse with schema information
    """
    return JSONResponse({
        "file_prefix": DRUG_SCHEMA.file_prefix,
        "required_columns": DRUG_SCHEMA.required_columns,
        "column_types": DRUG_SCHEMA.column_types,
        "example_row": DRUG_SCHEMA.example_row
    })
