"""
Routes for RPSR.csv file validation.
Handles single file, multiple files, folder, and ZIP uploads for RPSR files.
"""
...
from .validation_models import RPSR_SCHEMA

router = APIRouter(prefix="/rpsr", tags=["RPSR File Validation"])


@router.post("/validate-single")
async def validate_single_rpsr_file(file: UploadFile = File(...)) -> JSONResponse:
    """
    Validate a single RPSR.csv file.
    ...
    result = validate_single_file(file_bytes, file.filename, RPSR_SCHEMA)
...
async def validate_multiple_rpsr_files(files: List[UploadFile] = File(...)) -> JSONResponse:
    """
    Validate multiple RPSR.csv files.
    ...
    # Validate all files
    results = validate_multiple_files(file_data, RPSR_SCHEMA)
...
async def validate_rpsr_zip_file(file: UploadFile = File(...)) -> JSONResponse:
    """
    Validate RPSR.csv files from a ZIP archive.
    ...
    try:
        ...
        # Validate all files
        results = validate_multiple_files(files, RPSR_SCHEMA)
        summary = create_validation_summary(results)
...
async def validate_rpsr_folder(files: List[UploadFile] = File(...)) -> JSONResponse:
    """
    Validate all RPSR.csv files from a folder upload.
...
@router.get("/schema")
async def get_rpsr_schema() -> JSONResponse:
    """
    Get the RPSR file schema requirements.
    
    Returns:
        JSONResponse with schema information
    """
    return JSONResponse({
        "file_prefix": RPSR_SCHEMA.file_prefix,
        "required_columns": RPSR_SCHEMA.required_columns,
        "column_types": RPSR_SCHEMA.column_types,
        "example_row": RPSR_SCHEMA.example_row
    })
