"""
Routes for DEMO.csv file validation.
Handles single file, multiple files, folder, and ZIP uploads for DEMO files.
"""
...
from .validation_models import DEMO_SCHEMA

router = APIRouter(prefix="/demo", tags=["DEMO File Validation"])


@router.post("/validate-single")
async def validate_single_demo_file(file: UploadFile = File(...)) -> JSONResponse:
    """
    Validate a single DEMO.csv file.
    ...
    result = validate_single_file(file_bytes, file.filename, DEMO_SCHEMA)
...
async def validate_multiple_demo_files(files: List[UploadFile] = File(...)) -> JSONResponse:
    """
    Validate multiple DEMO.csv files.
    ...
    # Validate all files
    results = validate_multiple_files(file_data, DEMO_SCHEMA)
...
async def validate_demo_zip_file(file: UploadFile = File(...)) -> JSONResponse:
    """
    Validate DEMO.csv files from a ZIP archive.
    ...
    try:
        ...
        # Validate all files
        results = validate_multiple_files(files, DEMO_SCHEMA)
        summary = create_validation_summary(results)
...
async def validate_demo_folder(files: List[UploadFile] = File(...)) -> JSONResponse:
    """
    Validate all DEMO.csv files from a folder upload.
...
@router.get("/schema")
async def get_demo_schema() -> JSONResponse:
    """
    Get the DEMO file schema requirements.
    
    Returns:
        JSONResponse with schema information
    """
    return JSONResponse({
        "file_prefix": DEMO_SCHEMA.file_prefix,
        "required_columns": DEMO_SCHEMA.required_columns,
        "column_types": DEMO_SCHEMA.column_types,
        "example_row": DEMO_SCHEMA.example_row
    })
